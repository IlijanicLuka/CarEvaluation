...
def DecTreeReg(parameters, X_train,y_train,X_test, y_test):
    model = DecisionTreeClassifier(criterion= parameters[0],
                                   splitter= parameters[1],
                                   max_depth= parameters[2],
                                   min_samples_leaf = parameters[3],
                                   max_leaf_nodes= parameters[4])
...
while True:
    ...
    Param = DecTreeParRandomSearch()
    test = DecTreeReg(Param,X_train,y_train, X_test, y_test)
    k+=1
    if (test[8] > 0.999):
        BestScores = test
...
DecTree_BestMeanScores = [BestScores[0], BestScores[2], BestScores[4], BestScores[6], BestScores[8]]

DecTree_BestStdScores = [BestScores[1], BestScores[3], BestScores[5], BestScores[7], BestScores[9]]
