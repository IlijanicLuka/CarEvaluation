...
def KNNReg(parameters, X_train,y_train,X_test, y_test):
    model = KNeighborsClassifier(n_neighbors = parameters[0],
...
while True:
    ...
    Param = KNNParRandomSearch()
    test = KNNReg(Param,X_train,y_train, X_test, y_test)
    k+=1
    if (test[8] > 0.999):
        BestScores = test
...
KNN_BestMeanScores = [BestScores[0], BestScores[2], BestScores[4], BestScores[6], BestScores[8]]

KNN_BestStdScores = [BestScores[1], BestScores[3], BestScores[5], BestScores[7], BestScores[9]]
